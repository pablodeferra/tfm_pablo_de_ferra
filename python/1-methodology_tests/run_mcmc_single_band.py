...
# %%
# ===========================================================================
#  QUIJOTE 11 GHz standalone fit  –  EE and BB, with and without c term
# ===========================================================================
print("\n" + "#"*70)
print("  QUIJOTE 11 GHz standalone fit")
print("#"*70)

band_11 = '11'
results_11 = {m: {} for m in ['EE', 'BB']}

for ell_range_key, (ell_min, ell_max) in ELL_RANGES.items():
    print(f"\n{'='*60}")
    print(f"  11 GHz  |  ell range: {ell_min}-{ell_max}  ({ell_range_key})")
    print(f"{'='*60}")
...
# ==== Build 11 GHz table (both ell ranges in one table) ====
def build_latex_table_11ghz(results_11):
    """Two-column table (no_c / with_c) for 11 GHz, rows = EE/BB params."""
    lines = []
    lines.append(r'\begin{table}[htbp]')
    lines.append(r'\centering')
    lines.append(
        r'\caption{QUIJOTE 11\,GHz synchrotron fit: '
        r'$C_\ell = A_s\,(\ell/80)^{\alpha_s} [+ c_s]$ ($\beta_s$ frozen to 0). '
    ...
    lines.append(r'\toprule')
    lines.append(
        r'& \multicolumn{2}{c}{$\ell \in [30, 200]$} & \multicolumn{2}{c}{$\ell \in [30, 120]$} \\'
    )
    ...
    def _row(mode, param_label, param_key, rescale=False):
        ...
        for ell_key in ['full', 'short']:
            for fv in ['no_c', 'with_c']:
                key = f'{fv}_{ell_key}'
                summary = results_11[mode].get(key, {})
                if param_key is None:
                    # c term: find key dynamically
                    c_key = next((k for k in summary if k.startswith('c_sync')), None)
                    if c_key:
                        cols.append(fmt_val(summary, c_key, rescale=True))
                    else:
                        cols.append(r'\multicolumn{1}{c}{---}')
                else:
    ...
    # (chi2 rows removed by user request)
...
tex_11_path = os.path.join(tables_dir, f'sync_11ghz_{mask_name}{name_suffix}.tex')
table_11 = build_latex_table_11ghz(results_11)
with open(tex_11_path, 'w') as f:
    f.write(table_11)
print(f"\n[INFO] 11 GHz table saved: {tex_11_path}")
print(table_11)
