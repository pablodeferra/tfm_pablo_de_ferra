...
path_mask = mask_select['path']
use_simulated_maps = True
use_white_noise = True
out_path = '/home/pablo/Desktop/master/tfm/spectra/'
path_spectra = os.path.join(out_path, f'power_spectra_{mask_name}__test__.fits')
quijote_band_select = ['11']
wmap_band_select = []
planck_band_select = ['30']
...
# --- función worker ---
def _worker_one_sim(args):
    sim, data, band_list, path_mask, binning_params, use_white_noise, only_noise = args
    mask = hp.read_map(path_mask)
    b = create_binning(binning_params)
    return compute_all_power_spectra(
...
# --- función principal ---
def average_and_std_spectra_parallel(
    data, spectra_matrix, band_list, path_mask, binning_params,
    use_white_noise=False, n_sim=100, only_noise=False, n_workers=None
):
    """
    Compute average and std of spectra using parallel workers + progress bar.
    """
    ...
    # Prepara argumentos de cada simulación
    worker_args = [
        (sim, data, band_list, path_mask, binning_params, use_white_noise, only_noise)
        for sim in range(1, n_sim + 1)
    ]

    # Corre en paralelo con barra de progreso
    results = []
    with mp.Pool(processes=n_workers) as pool:
        for res in tqdm(pool.imap_unordered(_worker_one_sim, worker_args),
                        total=n_sim, desc="Simulations"):
            results.append(res)

    # Acumula
    for spectra_matrix_sim in results:
        for i in range(N_band):
            for j in range(N_band):
...
def save_avg_std_to_fits(avg_matrix, std_matrix, band_list, mask_name, out_path):
    """
    ...
    out_file = os.path.join(out_path, f'spectra_avg_std_{mask_name}.fits')
    hdu_list.writeto(out_file, overwrite=True)
    print(f"Saved avg+std spectra for mask \"{mask_name}\" to {out_file}")
...
# 5. Compute and save mean and std from 100 sky+noise simulations
# avg_matrix_skyplusnoise, std_matrix_skyplusnoise = average_and_std_spectra_parallel(
#     data=data,
#     spectra_matrix=spectra_matrix_read, 
#     band_list=band_list,
#     path_mask=path_mask,
...
# save_avg_std_to_fits(avg_matrix_skyplusnoise, std_matrix_skyplusnoise, band_list,
#                      mask_name="galcut10_avg_std100_skyplusnoise",
#                      out_path=out_path)



# # 6. Compute and save mean and std from 100 sky+noise simulations
# avg_matrix_skyplusnoise, std_matrix_skyplusnoise = average_and_std_spectra_parallel(
#     data=data,
#     spectra_matrix=spectra_matrix_read, 
#     band_list=band_list,
...
# save_avg_std_to_fits(avg_matrix_skyplusnoise, std_matrix_skyplusnoise, band_list,
#                      mask_name="galcut10_avg_std100_noise",
#                      out_path=out_path)
...
avg_std_skyplusnoise_spectra_matrix = read_avg_std_spectra_from_fits(avg_std_skyplusnoise_name, band_list)
avg_std_noise_spectra_matrix = read_avg_std_spectra_from_fits(avg_std_noise_name, band_list)

#%%

i = band_list.index("11")
j = band_list.index("11")

cl_11_11 = spectra_matrix[i, j]

print(cl_11_11)

#%%

q_11 = hp.read_map(data['QUIJOTE']['11']['path'], field=[0,1,2])
q_11 = np.where(q_11 == hp.UNSEEN, 0, q_11)
# hp.mollview(q_11[0], norm='hist')

ps_11_11 = cross_spectrum(mask, q_11, q_11, b, purify_e=True, purify_b=True)

#%%

noise_11 = hp.read_map(data['QUIJOTE']['11']['path_white_noise_simulations'], field=[0,1,2])
